...
- [新手上路](#新手上路)
  - [前置准备](#前置准备)
    ...
    - [2. 安装运行环境](#2-安装运行环境)
      ...
      - [2. .NET 8](#2-net-8)
      - [3. Python](#3-python)
    - [3. 确认模拟器和设备支持](#3-确认模拟器和设备支持)
    - [4. 正确设置模拟器分辨率](#4-正确设置模拟器分辨率)
    - [4. 下载正确的版本 TODO](#4-下载正确的版本-todo)
  - [开始使用](#开始使用)
    - [6. 开始使用](#6-开始使用)
      - [Windows](#windows)
      - [macOS](#macos)
...
| Mac 用户 | 由于作者缺少测试环境，MaaEOV 仅在理论上支持MacOS。如发行版无法正常运行，可准备任意可用的MaaFramework项目，替换`resource`文件夹和`interface.json`使用 |
| Linux 用户 | 同 Mac |
...
在 Windows 与 macOS 平台，MaaEOV 的发布包中已内置独立的 Python 环境。所有所需依赖也已预先打包到该环境中，因此用户无需额外安装 Python 或执行 pip install。

在 Linux 平台，由于无法以相同方式提供可移植的内置 Python 环境，当前版本 不会自动创建虚拟环境或安装依赖。因此，**Linux 用户需要在自己的 Python 环境中手动安装依赖。**

如果您没有对 Python 环境做特别自定义，可以使用下方命令安装依赖（假设您的默认 Python 为 3.10+）：
...
- 您的系统需要安装 **Python 版本 ≥ 3.10**。这是 MaaEOV 启动和管理其内部环境所必需的。
...
| 需要adb功能 | 是 | 是 | 是 |
| 备注 | MuMu 模拟器表现较为稳定 | [暂不支持 PlayCover](https://github.com/MaaXYZ/MaaFramework/issues/405) |  |
...
>[!WARNING]
>
>1. 注意修改分辨率后模拟器主页应该是**横屏（平板版）**，不要选成竖屏（手机版）！
>2. 在游戏的 **设置 → 画面** 中，请确保 **「低分辨率」选项处于关闭状态**。开启该选项会导致功能异常。
>
><img width="1678" height="944" alt="ScreenShot_2025-11-26_064436_910" src="https://github.com/user-attachments/assets/c7d95899-8c17-48c9-a6d7-c53f9da871fb" />
...
<!-- 国内用户也可通过 [Mirror 酱](https://mirrorchyan.com/zh/download?rid=MaaEOV&source=m9agh-md3) 高速下载。 -->

<div align="center">

| | Windows | macOS | Linux |
| :---: | :---: | :---: | :---: |
| 您需要下载 | `MaaEOV-win-x86_64-vXXX.zip` 或 `MaaEOV-win-aarch64-vXXX.zip`<br>取决于您的架构 | `MaaEOV-macos-x86_64-vXXX.tar.gz` 或 `MaaEOV-macos-aarch64-vXXX.tar.gz`<br>取决于您的架构 | `MaaEOV-linux-x86_64-vXXX.tar.gz` 或 `MaaEOV-linux-aarch64-vXXX.tar.gz`<br>取决于您的架构 |
...
<details>
  ...
  <blockquote>
    ...
    <ul>
      <li>若使用 Intel X86 处理器，请下载 <code>MaaEOV-macos-x86_64-vXXX.tar.gz</code></li>
      <li>若使用 Apple Silicon 系列如：M1、M2 等 ARM 架构处理器，请下载 <code>MaaEOV-macos-aarch64-vXXX.tar.gz</code></li>
    </ul>
  </blockquote>
</details>
...
1. 打开终端，解压分发的压缩包：
    ...
    **选项1：解压到系统目录（需要管理员权限）**
    ...
    ```shell
    sudo mkdir -p /usr/local/bin/MaaEOV
    sudo tar -xzf <下载的MaaEOV压缩包路径> -C /usr/local/bin/MaaEOV
    ```
    ...
    **选项2：解压到用户目录（推荐，无需sudo）**
    ...
    ```shell
    mkdir -p ~/MaaEOV
    tar -xzf <下载的MaaEOV压缩包路径> -C ~/MaaEOV
    ```

2. 进入解压目录并运行程序：

    ```shell
    cd /usr/local/bin/MaaEOV
    ./MaaPiCli
    ```
...
sudo xattr -rd com.apple.quarantine /usr/local/bin/MaaEOV/MaaPiCli
# 或用户目录版本：xattr -rd com.apple.quarantine ~/MaaEOV/MaaPiCli

# 方案2：添加到 Gatekeeper 白名单
sudo spctl --add /usr/local/bin/MaaEOV/MaaPiCli
# 或用户目录版本：spctl --add ~/MaaEOV/MaaPiCli

# 方案3：一次性处理整个目录
sudo xattr -rd com.apple.quarantine /usr/local/bin/MaaEOV/*
# 或用户目录版本：xattr -rd com.apple.quarantine ~/MaaEOV/*
```
...
<details open>
  ...
  <p></p>
  <blockquote>
    <img alt="主界面" src="https://github.com/user-attachments/assets/ab4460b4-0cb6-4413-986d-3761d8509b66" />
  </blockquote>
</details>

在主界面中，可看到 **`资源类型`** **`任务列表`** **`任务设置`** **`任务说明`** **`连接`** **`日志`** 六大板块。

MaaEOV 运行任务时，无法修改主界面的部分设置，如 `连接` 板块。
...
**`任务列表`**
  ...
  <details>
    ...
    <blockquote>
      <ul>
        <img alt="tasks" src="https://github.com/user-attachments/assets/7e4619c5-eb02-463b-b938-812fa1956325" />
      </ul>
    </blockquote>
  </details>
...
- [MaaPiCli操作说明](./MaaPiCli.md)——介绍了 MaaPiCli 使用方法
