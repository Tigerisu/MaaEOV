...
MaaEOV 可以自动检测**已启动**模拟器提供的 ADB 并自动填充 ADB 路径、连接地址和连接配置。

检测失败时，若您确定正在使用的模拟器和连接地址正确，请尝试使用 UAC 管理员权限启动 MaaEOV 并再次检测。若仍失败，请[手动配置](#手动配置)。
...
ADB 路径是指 MaaEOV 连接模拟器所需的 ADB 可执行文件路径。ADB 可执行文件一般由模拟器提供，您也可以自行安装谷歌提供的 ADB。
...
推荐直接解压到 MaaEOV 文件夹下，这样可以直接在 ADB 路径中填写 `.\platform-tools\adb.exe`，也可以随着 MaaEOV 文件夹一起移动。
...
> [!NOTE]
> 若需要多开模拟器同时操作，可将 MaaEOV 文件夹复制多份，使用 **不同的 MaaEOV**、**同一个 adb.exe**、**不同的连接地址** 来进行连接。
